...
TEMPLATE_NAME="pg-ctrls-hyd-db-Backup"
PLAYBOOK="playbook-backup-postgres.yaml"
AZ_BLOB_URL="https://prodmobiusdbbkp.blob.core.windows.net/postgresdb"
AZ_BLOB_TOKEN="svxxxx"
...
BASE_BACKUP_DIR="/data/postgresdb"
...
JOB_TEMPLATE_RESPONSE=$(curl -s -X POST "http://${NODE_IP}:${NODE_PORT}/api/v2/job_templates/" \
  ...
  -d '{
    "name": "'"${TEMPLATE_NAME}"'",
    "description": "This is a job template for PostgreSQL backup with machine credentials",
    "job_type": "run",
